...
def stop_procs(work, work_type):
    global proc_list

    removed_proc_index = []
    for proc_index in range(len(proc_list)):
        proc = proc_list[proc_index]
        print(proc.spec["work_type"], work_type)
        print(proc.spec["work_group_no"], work["work_group_no"])
        print(proc.spec["work_no"], work["work_no"])
        if proc.spec["work_type"] == work_type and \
                proc.spec["work_group_no"] == work["work_group_no"] and \
                proc.spec["work_no"] == work["work_no"]:
